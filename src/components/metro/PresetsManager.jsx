...
class PresetsManager extends Component {
	presetEditorRef = React.createRef();
	importDialogRef = React.createRef();
	...
	onPresetEdit(e, idx) {
		...
		if (idx !== undefined) {
			...
			this.presetEditorRef.current.edit(p, this.state.userPresets[idx].presetId, this.state.userPresets[idx].title)
		}
		else {
			this.presetEditorRef.current.edit(p)
		}
	}

	onImportBtn(e) {
		this.importDialogRef.current.open()
	}
	...
	onPresetsImport(presets) {
		...
		this.importDialogRef.current.close();
			...
			<SimplePanel className="PresetsManager" title={Tr("Presets")}>
				<Container>
					...
					<Row style={{ justifyContent: 'center' }}>
						<EditPresetModal ref={this.presetEditorRef} onDeleteBtn={(preset) => this.onPresetDelete(preset)} onSaveBtn={(e, idx) => this.onPresetEdit(e, idx)} onSave={(title, preset) => this.onSavePreset(title, preset)} />
						<ImportDialog ref={this.importDialogRef} onImportBtn={() => this.onImportBtn()} onJsonReady={(presets) => this.onPresetsImport(presets)}/>
